App.controller('DashboardCtrl', function($http,$filter,$compile, DTDefaultOptions,DTOptionsBuilder, DTColumnBuilder,$scope,$location, Flash, $uibModal, $routeParams, API, Helpers,$filter,$timeout) {
	...
	$scope.urlFilterParameters = function() {
		...
		if ($routeParams.dashboard_type) {
			$scope.filter_data['dashboard_type'] = $routeParams.dashboard_type;
		}

		if ($routeParams.nationality) {
			$scope.filter_data['nationality'] = $routeParams.nationality;
		}

		if ($routeParams.station) {
			$scope.filter_data['station'] = $routeParams.station;
		}

		if ($routeParams.station_shift) {
			$scope.filter_data['station_shift'] = $routeParams.station_shift;
		}

		$scope.station_shifts_list = [];
		API.GET('workshift/station-shifts-list',{day: $routeParams.day,station: $routeParams.station}).then(function(d){
			$scope.station_shifts_list = d.data;
		});
		if ($routeParams.day) {
			$scope.filter_data['day'] = $routeParams.day;
		}else {
			if ($scope.filter_data['dashboard_type'] == 'workshifts') {
				// Set current workshift day
				var current_day = $filter('date')(new Date,'dd/MM/yyyy');
				$scope.current_workshift_day = 1;
				var first_workshift_day = new Date;
				var last_workshift_day_k = null;
				var days_i = 0;
				angular.forEach(window.workshift_days_greg,function(v,day_k){
					if (days_i == 0) {
						first_workshift_day = new Date(v);
					}
					if (v == current_day) {
						$scope.current_workshift_day = window.workshift_days[day_k];
					}
					days_i++;
					last_workshift_day_k = day_k;
				});
				if ($scope.current_workshift_day == 1 && (new Date).getTime() > (new Date(first_workshift_day)).getTime()) {
					$scope.current_workshift_day = window.workshift_days[last_workshift_day_k];
				}
				$scope.filter_data['day'] = $scope.current_workshift_day;
			}else {
				$scope.filter_data['day'] = 1;
			}
		}
